...
public class MD5 {
    ...
    /**
     * 以标准MD5文件的形式返回一个目录下所有文件的MD5值。在服务器上默认使用threadNum个核；在个人电脑上默认使用全部核
     * @param inputDir 输入目录的绝对路径
     */
    public static void getMD5FromDir(String inputDir, int threadNum){
        long start = System.nanoTime();
        int numThreads = Math.min(Runtime.getRuntime().availableProcessors(), threadNum);
        ...
        List<Pair<Path, String>> results = utils.CommandUtils.run_commands(callableList, numThreads);
        StringBuilder sb = new StringBuilder();
        try (BufferedWriter bw = IOTool.getBufferedWriter(new File(inputDir, "md5.txt"))) {
        ...
        System.out.println("completed in " + String.format("%.4f", Benchmark.calculateTimeSpanMinutes(start)) + " minutes");
    }
    ...
    public static void checkMD5(String inputMD5File){
        ...
        String line;
        try(BufferedReader br=IOTool.getBufferedReader(inputMD5File)){
        ...
        for (String md5Path : md5ValuePath){
            temp = StringUtils.split(md5Path, " ");
            String md5 = temp[0];
            String path = new File(inputDir, temp[1]).getAbsolutePath();
